...
авторов из карточки как есть.

Модуль жил в пакете sigstats (вкладка «Поиск пакетов»); вкладку вырезали, а
авто-статистика в SiQuesterHYX осталась — поэтому клиент переехал сюда и ни от
чего, кроме requests, не зависит.
"""
...
def _name_variants(name: str) -> list[str]:
    ...
    Каталоги паков обрезают пробелы в заголовке, а SIStatistics сверяет
    ВНУТРЕННЕЕ имя пакета из content.xml — буква-в-букву, включая хвостовой/
    ведущий пробел (авторы часто оставляют «… от Автор »). Перебираем разумные
    варианты, чтобы не терять статистику из-за таких расхождений.
    """
    base = name or ""
    variants = [
        base,
...
def _query(session: requests.Session, name: str, authors_q: str) -> dict | None:
    url = (
        f"{STATS_BASE}/games/packages/stats"
...
def get_package_stats(
    ...
    Если точное имя не находится, пробуем варианты с пробелами (см.
    _name_variants).
    """
    authors_q = "".join(
